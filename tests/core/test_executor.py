...
async def test_result_under_the_cap_is_returned_untouched():
    session, original = _session([[1], [2], [3]])

    result = await execute(session, "SELECT n FROM t", max_rows=100)
...
async def test_result_exactly_at_the_cap_is_not_truncated():
    """The cap is inclusive: len(rows) > max_rows triggers truncation."""
    session, original = _session([[i] for i in range(10)])

    result = await execute(session, "SELECT n FROM t", max_rows=10)
...
async def test_result_over_the_cap_is_truncated_and_warned():
...
async def test_truncation_preserves_columns_and_timing():
    session, original = _session([[i] for i in range(5)], columns=("a",))

    result = await execute(session, "SELECT a FROM t", max_rows=2)

    assert result.columns == ["a"]
    assert result.execution_time_ms == original.execution_time_ms


async def test_truncation_appends_to_existing_warnings():
    ...
    result = await execute(session, "SELECT n FROM t", max_rows=2)

    assert result.warnings[0] == "adapter said something"
    assert any("truncated" in w for w in result.warnings[1:])


async def test_sql_is_passed_through_to_the_adapter():
