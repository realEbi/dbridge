...
outcome: interrupted work raises `CancelledError`, and work that already finished
keeps its result. Cancellation must leave the Session usable and cannot interrupt
another request. Earlier statements' effects are retained; cancellation does not
...
The async Adapter cancellation contract is provisional until MySQL implements it
using a native async driver; see [backlog 019](../backlog/019-mysql-adapter.md).
This decision does not choose streaming, pagination, transactions, pooling, or
network transport behavior. Result fetching still materializes all rows before
the response cap. DuckDB metadata may observe a different transaction context
