...
- [x] 1.1 Add a MySQL 8.4 compose file with a seed script to the test tree. The
      ...
      count is 1,000,000.
- [x] 1.2 Add the `tests/adapters/mysql/` fixtures. The server comes from
...
- [x] 2.1 Change the `mysql` extra to `aiomysql>=0.3.2,<0.4` plus `cryptography`.
...
- [x] 2.2 Register `mysql` in `adapters/registry.py` through a loader that imports
...
- [x] 3.1 Implement config parsing: host and port defaults, a numeric-string
...
- [x] 3.2 Implement the `_Channel`. It holds one aiomysql connection, a FIFO lock,
...
- [x] 4.1 Implement the control-connection registry keyed by `(host, port, user)`.
      ...
      - the control connection disappears from the process list after the last
        disconnect
- [x] 4.2 Implement cancellation on both Channels. Return a finished result.
...
- [x] 5.1 Classify requests with sqlglot's MySQL dialect. Only a single parsed
...
- [x] 5.2 Release capped killable statements through the kill path, discarding
...
- [x] 6.1 Implement `disconnect()`: cancel through the kill path, close both
...
- [x] 7.1 Declare the `database` Scope Level. Implement the default Scope Path
...
- [x] 7.2 Implement `getTableSchema`: ordered columns with full type,
...
- [x] 8.1 Add a real-server subprocess test for the full DSP flow on a MySQL
      Profile:
...
- [x] 9.1 Update the README. Cover installing `dbridge[mysql]` and the MySQL
...
- [x] 9.2 Update `docs/architecture.md` (the queries and adapters section, the
      ...
      provisional.
- [x] 9.3 Update `docs/development.md` with `make mysql-up`, `make test-mysql`, and
      ...
      check.
- [x] 9.4 Update the milestone 3 roadmap text: MySQL is shipped, and ADR-0003 is
...
- [x] 10.1 Run the full verification in the topic worktree:
      - `make check`
...
- [x] 10.2 Synchronize the delta specs into `openspec/specs/`: the new
